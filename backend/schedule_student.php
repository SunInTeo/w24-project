...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($user_id) || empty($presentation_type)) {
            echo json_encode(['status' => 'error', 'message' => 'User ID and presentation type are required.']);
            http_response_code(400);
            exit;
        }
        ...
        if ($presentation_type === 'Essay') {
            $stmt = $pdo->prepare("
                UPDATE Users 
                SET essay_presentation_datetime = NULL 
                WHERE user_id = :user_id
            ");
            $stmt->execute(['user_id' => $user_id]);
            ...
            $stmt = $pdo->prepare("
                UPDATE Users 
                SET project_presentation_datetime = NULL 
                WHERE user_id = :user_id
            ");
        ...
        echo json_encode(['status' => 'success', 'message' => 'Presentation time deleted successfully.']);
    } else {
